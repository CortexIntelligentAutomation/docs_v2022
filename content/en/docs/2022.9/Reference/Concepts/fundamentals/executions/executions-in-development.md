...
A [flow][] can be debugged in [Cortex Studio][] by clicking the [Start an execution][] button or by making an [API call][API call: Cortex Studio] to [Cortex Studio][], [providing input variables][Providing Input Variables] to the flow, if required; this will [validate the flow][Validating a Flow] a   nd then begin an execution.
